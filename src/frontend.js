...
(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const s of o.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&n(s)}).observe(document,{childList:!0,subtree:!0});function r(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(a){if(a.ep)return;a.ep=!0;const o=r(a);fetch(a.href,o)}})();const ce=Symbol("raw"),Oe=Symbol("trusted");class G{constructor(t){this[Oe]=!0,this.value=String(t)}toString(){return this.value}}function A(e){return e==null||e===!1?new G(""):new G(e)}function Ne(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function fe(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;")}function xe(e){return e==null||e===!1?"":e===!0?"true":e instanceof G?e.value:e&&typeof e=="object"&&ce in e?e[ce]:Array.isArray(e)?e.map(xe).join(""):Ne(e)}function i(e,...t){let r=e[0];for(let n=0;n<t.length;n++)r+=xe(t[n])+e[n+1];return new G(r)}function Q(e){const t=e instanceof G?e.value:String(e).trim(),r=document.createElement("template");return r.innerHTML=t,r.content.firstElementChild}const we="fitness-coach:state",ke={user:null,currentWorkout:null,currentProgram:null,currentTab:"dashboard",ui:{theme:"system",activeModal:null,lastVisitedTab:null},workout:{activeExerciseIndex:0,restTimerEndTime:null,workoutStartTime:null,exerciseHistory:{},modifications:{added:[],deleted:[]},pendingNotes:""},mealTracking:{searchResults:[],selectedFoods:[],currentMealType:null,scannerActive:!1},pastWorkout:{date:null,programId:null,programDayId:null,exercises:[]},analytics:{lastFetch:null,data:null},aiCoach:{conversationHistory:[],lastInsight:null}},Se=["ui"];function ne(e,t){return t?t.split(".").reduce((r,n)=>r==null?r:r[n],e):e}function \$e(e,t,r){if(!t)return r;const n=t.split("."),a=n.pop(),o=n.reduce((s,l)=>((s[l]==null||typeof s[l]!="object")&&(s[l]={}),s[l]),e);return o[a]=r,e}function pe(e){return e==null||typeof e!="object"?e:JSON.parse(JSON.stringify(e))}function Ve(){try{const e=localStorage.getItem(we);return e?JSON.parse(e):{}}catch{return{}}}function Ge(e){try{const t={};for(const r of Se){const n=ne(e,r);n!==void 0&&\$e(t,r,n)}localStorage.setItem(we,JSON.stringify(t))}catch{}}class Ue{constructor(t){this.data=pe(t),this.subscribers=new Map;const r=Ve();for(const[n,a]of Object.entries(r))this.data[n]={...this.data[n],...a}}get(t){return ne(this.data,t)}set(t,r){const n=this.get(t);if(n===r)return;\$e(this.data,t,r),Se.some(o=>t===o||t.startsWith(\`\${o}.\`))&&Ge(this.data),this.notify(t,r,n);const a=t.split(".");for(let o=a.length-1;o>0;o--){const s=a.slice(0,o).join(".");this.notify(s,this.get(s),null)}}merge(t,r){const n=this.get(t)||{};this.set(t,{...n,...r})}reset(t){const r=ne(ke,t);this.set(t,pe(r))}subscribe(t,r){this.subscribers.has(t)||this.subscribers.set(t,new Set);const n=this.subscribers.get(t);return n.add(r),()=>n.delete(r)}notify(t,r,n){const a=this.subscribers.get(t);if(a)for(const o of a)try{o(r,n)}catch(s){console.error(\`Subscriber error for "\${t}":\`,s)}}}const w=new Ue(ke);typeof window<"u"&&(window.__store=w);const Te="/api",Ke=3e4,je=6e4,j=new Map;class ee extends Error{constructor(t,{status:r=0,code:n=null,details:a=null}={}){super(t),this.name="ApiError",this.status=r,this.code=n,this.details=a}}async function H(e,t={}){var W;const{method:r="GET",body:n,headers:a={},cache:o=!1,timeout:s=Ke,signal:l}=t,c=\`\${Te}\${e}\`,u=\`\${r} \${c}\`;if(r==="GET"&&o){const M=j.get(u);if(M&&Date.now()-M.at<je)return M.data}const m=new AbortController,y=setTimeout(()=>m.abort(),s);l&&l.addEventListener("abort",()=>m.abort(),{once:!0});const S={method:r,headers:{"Content-Type":"application/json",...a},signal:m.signal};n!=null&&(n instanceof FormData?(delete S.headers["Content-Type"],S.body=n):typeof n=="string"?S.body=n:S.body=JSON.stringify(n));let _;try{_=await fetch(c,S)}catch(M){throw clearTimeout(y),M.name==="AbortError"?new ee("Request timed out",{code:"TIMEOUT"}):new ee("Network error",{code:"NETWORK",details:M.message})}clearTimeout(y);const E=(W=_.headers.get("content-type"))==null?void 0:W.includes("application/json"),b=E?await _.json().catch(()=>({})):await _.text();if(!_.ok){const M=E&&(b==null?void 0:b.error)||\`Request failed (\${_.status})\`;throw new ee(M,{status:_.status,code:b==null?void 0:b.code,details:b==null?void 0:b.details})}return r==="GET"&&o&&j.set(u,{data:b,at:Date.now()}),b}function Ze(e=""){const t=\`GET \${Te}\${e}\`;for(const r of j.keys())r.startsWith(t)&&j.delete(r)}const p={get:(e,t)=>H(e,{...t,method:"GET"}),post:(e,t,r)=>H(e,{...r,method:"POST",body:t}),put:(e,t,r)=>H(e,{...r,method:"PUT",body:t}),patch:(e,t,r)=>H(e,{...r,method:"PATCH",body:t}),delete:(e,t)=>H(e,{...t,method:"DELETE"}),request:H,invalidateCache:Ze},ge={success:"fa-check-circle",error:"fa-exclamation-circle",warning:"fa-exclamation-triangle",info:"fa-info-circle"},Je=3e3;let \$=null;function Qe(){return \$&&document.body.contains(\$)||(\$=document.getElementById("toast-container"),\$||(\$=document.createElement("div"),\$.id="toast-container",\$.setAttribute("aria-live","polite"),\$.setAttribute("aria-atomic","true"),document.body.appendChild(\$))),\$}function O(e,t={}){var m;const{type:r="info",duration:n=Je,icon:a,action:o}=t,s=a?\`fa-\${a}\`:ge[r]||ge.info,l=Q(i\`
    <div class="toast toast-\${fe(r)}" role="alert">
      <i class="fas \${s} toast-icon" aria-hidden="true"></i>
      <div class="toast-message">\${e}</div>
      \${o?i\`<button class="btn btn-sm btn-ghost toast-action">\${o.label}</button>\`:""}
    </div>
  \`);Qe().appendChild(l),o!=null&&o.onClick&&((m=l.querySelector(".toast-action"))==null||m.addEventListener("click",()=>{o.onClick(),u()}));let c=!1;const u=()=>{c||(c=!0,l.style.animation="slide-out-right var(--transition-base) forwards",l.addEventListener("animationend",()=>l.remove(),{once:!0}))};return n>0&&setTimeout(u,n),u}const g={show:(e,t)=>O(e,t),success:(e,t)=>O(e,{...t,type:"success"}),error:(e,t)=>O(e,{...t,type:"error",duration:(t==null?void 0:t.duration)??5e3}),warning:(e,t)=>O(e,{...t,type:"warning"}),info:(e,t)=>O(e,{...t,type:"info"})},B=[];function Ye(){let e=document.getElementById("modal-root");return e||(e=document.createElement("div"),e.id="modal-root",document.body.appendChild(e)),e}function Xe(e){const t=e.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])');if(t.length===0)return null;const r=t[0],n=t[t.length-1],a=o=>{o.key==="Tab"&&(o.shiftKey&&document.activeElement===r?(o.preventDefault(),n.focus()):!o.shiftKey&&document.activeElement===n&&(o.preventDefault(),r.focus()))};return e.addEventListener("keydown",a),r.focus(),()=>e.removeEventListener("keydown",a)}function C(e){var de;const{title:t,content:r,actions:n=[],size:a="default",dismissable:o=!0,onOpen:s,onClose:l}=e,c=a==="wide"?"modal-wide":a==="fullscreen"?"modal-fullscreen":"",u=n.length?i\`
        <div class="modal-footer">
          \${n.map((h,I)=>i\`
            <button
              class="btn \${h.variant||(h.primary?"btn-primary":"btn-outline")}"
              data-modal-action="\${I}"
            >\${h.label}</button>
          \`)}
        </div>
  ...
  \`),S=y.querySelector(".modal"),_=y.querySelector(".modal-body");r instanceof Node?_.appendChild(r):r!=null&&(_.innerHTML=String(r));const E=document.activeElement;let b=!1,W;const M=new Promise(h=>W=h),L=h=>{if(b)return;b=!0;const I=B.indexOf(P);I>=0&&B.splice(I,1),X==null||X(),y.style.animation="fade-out var(--transition-fast) forwards",S.style.animation="slide-up var(--transition-fast) reverse forwards",y.addEventListener("animationend",()=>{var q;y.remove(),(q=E==null?void 0:E.focus)==null||q.call(E)},{once:!0}),B.length===0&&(document.body.style.overflow=""),l==null||l(h),W(h)},P={element:S,backdrop:y,close:L,promise:M};y.querySelectorAll("[data-modal-action]").forEach(h=>{const I=parseInt(h.getAttribute("data-modal-action"),10),q=n[I];h.addEventListener("click",async()=>{q.onClick?await q.onClick(P):L(q.value??I)})}),(de=y.querySelector("[data-modal-close]"))==null||de.addEventListener("click",()=>L()),o&&y.addEventListener("click",h=>{h.target===y&&L()});const le=h=>{h.key==="Escape"&&o&&B[B.length-1]===P&&L()};document.addEventListener("keydown",le);const We=L;P.close=h=>{document.removeEventListener("keydown",le),We(h)},Ye().appendChild(y),document.body.style.overflow="hidden",B.push(P);const X=Xe(S);return s==null||s(P),P}function Y(e,t={}){const{title:r="Are you sure?",confirmLabel:n="Confirm",cancelLabel:a="Cancel",confirmVariant:o="btn-primary"}=t;return new Promise(s=>{C({title:r,content:i\`<p>\${e}</p>\`,actions:[{label:a,variant:"btn-outline",onClick:l=>l.close(!1)},{label:n,variant:o,primary:!0,onClick:l=>l.close(!0)}],onClose:l=>s(l===!0)})})}function U({value:e,max:t,size:r=100,label:n,unit:a,color:o}){const s=Math.max(0,Math.min(100,e/(t||1)*100)),l=(r-16)/2,c=2*Math.PI*l,u=c-s/100*c,m=o?\`stroke: \${fe(o)};\`:"";return i\`
  ...
  \`}const K=new Map;function f(e,t,r){K.set(e,{legacyGlobal:t,handler:r})}function ue(){for(const[t,{legacyGlobal:r,handler:n}]of K){const a=window[r];window[r]=function(...s){try{return n(...s)}catch(l){if(console.error(\`[bridge] "\${r}" handler failed, falling back to legacy:\`,l),typeof a=="function")return a.apply(this,s);throw l}},window[r].__legacy=a,window[r].__modular=!0}const e=window.switchTab;typeof e=="function"&&!e.__bridged&&(window.switchTab=function(r,...n){return w.set("currentTab",r),e.call(this,r,...n)},window.switchTab.__bridged=!0,window.switchTab.__legacy=e),console.info(\`[bridge] Installed \${K.size} modular screen handler(s):\`,Array.from(K.keys()).join(", "))}function et(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ue,{once:!0}):ue()}const tt=\`    <div class="card">
...
\`,rt={hypertrophy:"#hypertrophy-section",strength:"#strength-section",endurance:"#endurance-section",cardio:"#cardio-section",comparison:"#comparison-section",variables:"#variables-section"};function nt(e){e.innerHTML=tt,e.addEventListener("click",t=>{var o;const r=t.target.closest("[data-learn-scroll]");if(!r)return;t.preventDefault();const n=r.getAttribute("data-learn-scroll"),a=rt[n];a&&((o=e.querySelector(a))==null||o.scrollIntoView({behavior:"smooth",block:"start"}))})}function at(){const e=document.getElementById("learn");e&&nt(e)}const _e=2.20462,Me=1/2.54;function ze(e){return e==null?0:Math.round(e*_e*10)/10}function it(e){return e==null?0:Math.round(e/_e*100)/100}function ot(e){return e*Me}function st(e){return e/Me}function lt(e){const t=Math.round(ot(e)),r=Math.floor(t/12),n=t-r*12;return{feet:r,inches:n}}function dt(e,t){return st((e||0)*12+(t||0))}function ct(){const e=w.get("user");return e&&e.measurement_system||"metric"}function Ce(){return ct()==="imperial"}function Ee(){return Ce()?"lbs":"kg"}function Re(e){return e==null?0:Ce()?ze(e):e}function T(e,t={}){const{unit:r=!0,precision:n=1}=t;if(e==null)return"N/A";const a=Re(e),o=Ee();let s;return Math.abs(a)>=1e3?s=Math.round(a).toLocaleString():a%1===0?s=String(a):s=a.toFixed(n),r?\`\${s} \${o}\`:s}function R(e){if(e==null||e<0)return"00:00";const t=Math.abs(Math.floor(e)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60,o=s=>String(s).padStart(2,"0");return r>0?\`\${o(r)}:\${o(n)}:\${o(a)}\`:\`\${o(n)}:\${o(a)}\`}function z(e,t={month:"short",day:"numeric",year:"numeric"}){if(!e)return"";const[r,n,a]=String(e).split("T")[0].split("-").map(Number);return!r||!n||!a?e:new Date(r,n-1,a).toLocaleDateString("en-US",t)}function pt(e){if(e==null||Number.isNaN(e))return"—";const t=Math.round(e*10)/10;return t===0?"0%":\`\${t>0?"↑":"↓"} \${Math.abs(t)}%\`}function gt(e){return e==null?"":e<=2?"😴":e<=4?"🙂":e<=6?"💪":e<=8?"🔥":"🥵"}const ut={consistency:"🔥",strength:"💪",volume:"📊",milestone:"⭐"},mt={consistency:"Consistency",strength:"Strength",volume:"Volume",milestone:"Milestones"},yt={bronze:"tier-bronze",silver:"tier-silver",gold:"tier-gold",platinum:"tier-platinum"};function ht(e){const t={};for(const r of e){const n=r.category||"other";t[n]||(t[n]=[]),t[n].push(r)}return t}function vt(e,t){return!t||t===0?{text:"First PR!",isFirst:!0}:{text:\`+\${((e-t)/t*100).toFixed(1)}%\`,isFirst:!1}}function bt({currentWeeks:e,longestWeeks:t,totalWorkouts:r,totalPRs:n,earnedCount:a}){return i\`
  ...
  \`}function ft(e){return!e||e.length===0?i\`
    ...
    <div class="table-container">
      <table class="data-table">
        ...
        <tbody>
          \${e.map(t=>{const{text:r,isFirst:n}=vt(t.record_value,t.previous_value);return i\`
              <tr>
                ...
                <td>\${z(t.achieved_at)}</td>
              </tr>
            \`})}
        </tbody>
      </table>
    </div>
  \`}function xt(e){const t=Object.keys(e);return t.length===0?i\`
      <div class="empty-state">
    ...
    <div class="stack stack-lg">
      \${t.map(r=>{const n=e[r];return i\`
          <div>
            ...
            <div class="grid grid-cols-auto">
              \${n.map(a=>i\`
                <div class="achievement-card \${yt[a.tier]||""}">
                  <div class="achievement-icon">\${a.icon||"🏅"}</div>
                  <div class="achievement-name">\${a.name}</div>
                  <div class="achievement-description">\${a.description}</div>
                  <div class="achievement-date">
                    <i class="fas fa-calendar"></i> \${z(a.earned_at)}
                  </div>
                </div>
              \`)}
            </div>
  ...
  \`}function wt(e,t,r){return!e.current_streak||e.current_streak<=0?"":i\`
    <div class="streak-card">
      <div class="streak-row">
        ...
        <div class="streak-metric">
          ...
          <div class="streak-value-small">\${z(e.streak_start_date)}</div>
        </div>
      </div>
      \${e.last_workout_date?i\`<div class="text-muted" style="font-size: var(--text-sm);">
            Last workout: \${z(e.last_workout_date)}
          </div>\`:""}
    </div>
  ...
  \`}function St(){return i\`
    <div class="card">
      <div class="grid grid-cols-auto">
        \${[1,2,3,4].map(()=>i\`<div class="skeleton skeleton-card"></div>\`)}
      </div>
    </div>
  \`}async function \$t(){const e=document.getElementById("achievements");if(e){e.innerHTML=String(St());try{const[t,r,n]=await Promise.all([p.get("/achievements"),p.get("/achievements/prs?limit=10"),p.get("/achievements/streak")]),a=t.earned||[],o=t.stats||{totalWorkouts:0,totalPRs:0},s=r.prs||[],l=n.streak||{},c=Math.floor((l.current_streak||0)/7),u=Math.floor((l.longest_streak||0)/7),m=ht(a);e.innerHTML=String(i\`
      <div class="card">
        ...
        \${bt({currentWeeks:c,longestWeeks:u,totalWorkouts:o.totalWorkouts||0,totalPRs:o.totalPRs||0,earnedCount:a.length})}
    ...
    \`)}catch(t){console.error("Error loading achievements:",t),e.innerHTML=String(kt(t.message))}}}function Tt(e){return i\`
    <div class="grid grid-cols-auto">
      <div class="stat-card">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value">\${R(e.total_time_seconds||0)}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">Avg Workout</div>
        <div class="stat-value">\${R(e.average_workout_time||0)}</div>
      </div>
    </div>
  \`}function _t(){return i\`
    <div class="cluster">
  ...
  \`}function Mt(e){const t=!!e.completed,r=t?"✓ Complete":"In Progress",n=t?"badge-success":"badge-warning",a=z(e.start_time,{weekday:"short",month:"short",day:"numeric",year:"numeric"}),o=e.total_weight_kg?T(e.total_weight_kg):"N/A";return i\`
    <div class="workout-card" id="workout-card-\${e.id}">
        ...
        <div class="workout-card-header-main">
          ...
          <div class="workout-card-metrics">
            <span class="text-muted">
              <i class="fas fa-clock"></i> \${R(e.total_duration_seconds)}
            </span>
            <span class="text-muted">
              <i class="fas fa-weight-hanging"></i> \${o}
            </span>
          </div>
        </div>
      ...
      <div class="workout-card-details" id="workout-details-\${e.id}" hidden>
        <div class="grid grid-cols-auto" style="gap: var(--space-3);">
          \${N("Training Time",R(e.total_duration_seconds))}
          \${N("Exercises",e.exercise_count||0)}
          \${N("Total Sets",e.total_sets||0)}
          \${N("Total Volume",o)}
          \${N("Effort Level",e.perceived_exertion?\`\${e.perceived_exertion}/10 \${gt(e.perceived_exertion)}\`:"Not rated")}
        </div>
  ...
  \`}function N(e,t){return i\`
    <div class="mini-stat">
      <div class="stat-label">\${e}</div>
      <div class="mini-stat-value">\${t}</div>
    </div>
  \`}function zt(e){return!e||e.length===0?i\`
      <div class="empty-state">
        <div class="empty-state-icon">🏋️</div>
  ...
  \`}function Rt(e){e.addEventListener("click",t=>{const r=t.target.closest("[data-legacy-call]");if(r){const l=r.getAttribute("data-legacy-call"),c=window[l];typeof c=="function"&&c();return}const n=t.target.closest("[data-tab]");if(n){const l=n.getAttribute("data-tab");typeof window.switchTab=="function"&&window.switchTab(l);return}const a=t.target.closest('[data-action="toggle-details"]');if(a){const l=a.getAttribute("data-workout-id"),c=document.getElementById(\`workout-details-\${l}\`),u=document.getElementById(\`workout-chevron-\${l}\`);c&&(c.hasAttribute("hidden")?(c.removeAttribute("hidden"),u==null||u.style.setProperty("transform","rotate(180deg)")):(c.setAttribute("hidden",""),u==null||u.style.removeProperty("transform")));return}const o=t.target.closest('[data-action="view-workout"]');if(o){t.stopPropagation();const l=parseInt(o.getAttribute("data-workout-id"),10);typeof window.viewWorkout=="function"&&window.viewWorkout(l);return}const s=t.target.closest('[data-action="delete-workout"]');if(s){t.stopPropagation();const l=parseInt(s.getAttribute("data-workout-id"),10);typeof window.deleteDashboardWorkout=="function"&&window.deleteDashboardWorkout(l);return}})}async function Pt(){const e=document.getElementById("dashboard");if(e){e.innerHTML=String(Ct());try{const[t,,r]=await Promise.all([p.get("/analytics/progress?days=30"),p.get("/programs"),p.get("/workouts?limit=5")]),n=t.overview||{},a=r.workouts||[];e.innerHTML=String(i\`
      <div class="stack stack-lg">
        <div class="card">
          <div class="card-header">
    ...
    \`),Rt(e)}catch(t){console.error("Error loading dashboard:",t),e.innerHTML=String(Et(t.message))}}}function It(e){const r=(e.measurement_system||"metric")==="imperial";let n="",a="";if(r&&e.height_cm){const s=lt(e.height_cm);n=s.feet,a=s.inches}const o=r&&e.weight_kg?Math.round(ze(e.weight_kg)*10)/10:e.weight_kg||"";return i\`
    <div class="stack stack-md">
      ...
      <div id="imperial-inputs" \${r?"":"hidden"}>
        ...
        <div class="form-group">
          ...
          <input type="number" id="profile-weight-lbs" class="input" value="\${o}" step="0.1" min="40" max="1100" />
        </div>
      </div>
      ...
      <div>
        ...
        <div class="stack stack-sm">
          \${[{id:"weekly",label:"Weekly Report",hint:"Sent every Monday morning"},{id:"monthly",label:"Monthly Report",hint:"Sent on the 1st of each month"},{id:"yearly",label:"Yearly Report",hint:"Sent on January 1st"}].map(s=>i\`
            <label class="report-pref-row">
              <input type="checkbox" id="profile-report-\${s.id}" data-report-toggle="\${s.id}" />
              <div>
                <strong>\${s.label}</strong>
                <div class="text-muted" style="font-size: var(--text-xs);">\${s.hint}</div>
              </div>
            </label>
          \`)}
        </div>
  ...
  \`}function At(e){const t=e.querySelector("#profile-system"),r=t==null?void 0:t.value,n=e.querySelector("#metric-inputs"),a=e.querySelector("#imperial-inputs");!n||!a||(r==="imperial"?(n.setAttribute("hidden",""),a.removeAttribute("hidden")):(a.setAttribute("hidden",""),n.removeAttribute("hidden")))}async function Lt(e){try{const r=(await p.get("/reports/preferences")).preferences||{},n={weekly:"weeklyReport",monthly:"monthlyReport",yearly:"yearlyReport"};for(const[a,o]of Object.entries(n)){const s=e.querySelector(\`#profile-report-\${a}\`);s&&(s.checked=!!r[o])}}catch(t){console.error("Failed to load report preferences:",t)}}async function qt(e){var t,r,n;try{return await p.put("/reports/preferences",{weeklyReport:!!((t=e.querySelector("#profile-report-weekly"))!=null&&t.checked),monthlyReport:!!((r=e.querySelector("#profile-report-monthly"))!=null&&r.checked),yearlyReport:!!((n=e.querySelector("#profile-report-yearly"))!=null&&n.checked)}),!0}catch(a){return console.error("Failed to save report preferences:",a),!1}}function Ht(e){g.info(\`Generating \${e} report preview…\`);const t=localStorage.getItem("token")||"",r=t?\`?token=\${encodeURIComponent(t)}\`:"";window.open(\`/api/reports/html/\${e}\${r}\`,"_blank")}async function Bt(e,t){const r=e.querySelector("#profile-name").value.trim(),n=parseInt(e.querySelector("#profile-age").value,10)||null,a=e.querySelector("#profile-gender").value,o=e.querySelector("#profile-system").value;let s,l;if(o==="imperial"){const c=parseFloat(e.querySelector("#profile-height-feet").value)||0,u=parseFloat(e.querySelector("#profile-height-inches").value)||0,m=parseFloat(e.querySelector("#profile-weight-lbs").value)||0;s=c||u?dt(c,u):null,l=m?it(m):null}else s=parseFloat(e.querySelector("#profile-height-cm").value)||null,l=parseFloat(e.querySelector("#profile-weight-kg").value)||null;try{const c=await t.put("/auth/user",{name:r,age:n,gender:a,height_cm:s,weight_kg:l,measurement_system:o});w.set("user",c.user),await qt(e),g.success("Profile updated");const u=window.loadUser;typeof u=="function"&&u()}catch(c){throw g.error(\`Error saving profile: \${c.message}\`),c}}function Dt(){const e=w.get("user")||{};return C({title:"User Profile",size:"default",content:String(It(e)),actions:[{label:"Cancel",variant:"btn-outline"},{label:"Save Profile",primary:!0,variant:"btn-primary",onClick:async r=>{try{await Bt(r.element,p),r.close("saved")}catch{}}}],onOpen:({element:r})=>{Lt(r),r.addEventListener("change",n=>{n.target.matches("#profile-system")&&At(r)}),r.addEventListener("click",n=>{const a=n.target.closest('[data-action="preview"]');a&&(n.preventDefault(),Ht(a.getAttribute("data-period")))})}})}const Ft=[{value:"Smith Machine",label:"Smith Machine",default:!0},{value:"Olympic Bar",label:"Olympic Bar",default:!0},{value:"Cable Trainer",label:"Cable Machine",default:!0},{value:"Leg Extension/Curl",label:"Leg Ext/Curl",default:!0},{value:"Pull-up Bar",label:"Pull-up Bar"},{value:"Landmine",label:"Landmine"},{value:"Dip Bars",label:"Dip Bars"},{value:"Rower",label:"Rower"},{value:"Treadmill",label:"Treadmill"},{value:"Stationary Bike",label:"Bike"}],Wt={1:"cardio-zone-low",2:"cardio-zone-low",3:"cardio-zone-mid",4:"cardio-zone-high",5:"cardio-zone-high"};function F(){return Pe()}function Ot(e){const t=e.active?i\`<span class="badge badge-success">ACTIVE</span>\`:i\`<button class="btn btn-outline btn-sm" data-action="activate" data-program-id="\${e.id}">Activate</button>\`;return i\`
  ...
  \`}async function Pe(){const e=document.getElementById("program");if(e){e.innerHTML=String(i\`
    <div class="stack stack-lg">
      <div class="card">
  ...
  \`);try{const r=(await p.get("/programs")).programs||[],n=r.find(a=>a.active);e.innerHTML=String(i\`
      <div class="stack stack-lg">
        ...
        <div class="card">
          ...
          \${r.length>0?i\`<div class="stack stack-sm">\${r.map(Ot)}</div>\`:i\`
                <div class="empty-state">
                  <div class="empty-state-icon">📋</div>
    ...
    \`),Nt(e)}catch(t){console.error("Error loading programs:",t),e.innerHTML=String(i\`
      <div class="card">
        <div class="empty-state">
          <div class="empty-state-icon">⚠️</div>
    ...
    \`)}}}function Nt(e){e.addEventListener("click",t=>{const r=t.target.closest("[data-action]");if(!r)return;const n=r.getAttribute("data-action"),a=parseInt(r.getAttribute("data-program-id"),10);switch(n){case"view":t.stopPropagation(),oe(a);break;case"activate":t.stopPropagation(),se(a);break;case"generate":Ae();break;case"create-manual":Le();break}})}function Vt(e){return i\`
  ...
  \`}function Gt(e){const t=e.cardio_sessions||[];return i\`
    <strong class="cardio-day-title">
      <i class="fas fa-fire"></i> Cardio Sessions
    </strong>
    <div class="stack stack-sm">
      \${t.map(r=>i\`
        <div class="cardio-session \${Wt[r.heart_rate_zone]||"cardio-zone-mid"}">
  ...
  \`}function Ut(e){return i\`
    <div class="program-day-card">
      ...
      \${e.is_cardio_day?Gt(e):Vt(e)}
    </div>
  \`}async function oe(e){try{const r=(await p.get(\`/programs/\${e}\`)).program,n=r.created_at?z(r.created_at,{year:"numeric",month:"short",day:"numeric"}):"Unknown";return C({title:"Program Details",size:"wide",content:String(i\`
      ...
      \`),actions:[{label:"Start Workout",primary:!0,variant:"btn-primary",onClick:o=>{o.close(),rr(e)}},...r.active?[]:[{label:"Set as Active",variant:"btn-secondary",onClick:async o=>{await se(e,{silent:!1}),o.close()}}],{label:"Delete",variant:"btn-danger",onClick:async o=>{if(await Y("Delete this program? This cannot be undone.",{title:"Delete program?",confirmLabel:"Delete",confirmVariant:"btn-danger"}))try{await p.delete(\`/programs/\${e}\`),g.success("Program deleted"),o.close(),F()}catch(l){g.error(\`Error deleting program: \${l.message}\`)}}}],onOpen:({element:o})=>{var s;(s=o.querySelector('[data-action="rename"]'))==null||s.addEventListener("click",()=>{Ie(e,r.name)})}})}catch(t){g.error(\`Error loading program: \${t.message}\`)}}function Ie(e,t){C({title:"Rename Program",size:"default",content:String(i\`
      <div class="form-group">
        <label class="form-label" for="rename-input">Program name</label>
        <input type="text" id="rename-input" class="input" value="\${t}" />
      </div>
    \`),actions:[{label:"Cancel",variant:"btn-outline"},{label:"Save Name",primary:!0,onClick:async r=>{const n=r.element.querySelector("#rename-input").value.trim();if(!n){g.warning("Please enter a program name");return}try{await p.patch(\`/programs/\${e}\`,{name:n}),g.success("Program renamed"),r.close(),F(),oe(e)}catch(a){g.error(\`Failed to rename: \${a.message}\`)}}}],onOpen:({element:r})=>{const n=r.querySelector("#rename-input");n==null||n.focus(),n==null||n.select()}})}async function se(e,t={}){try{await p.post(\`/programs/\${e}/activate\`),t.silent||g.success("Program activated"),F()}catch(r){g.error(\`Error activating program: \${r.message}\`)}}async function Kt(e){if(await Y("Are you sure you want to delete this program?",{title:"Delete program?",confirmLabel:"Delete",confirmVariant:"btn-danger"}))try{await p.delete(\`/programs/\${e}\`),g.success("Program deleted"),F()}catch(r){g.error(\`Error deleting program: \${r.message}\`)}}function Ae(){C({title:"Generate AI Program",size:"default",content:String(i\`
      <div class="stack stack-md">
        <div class="form-group">
    ...
    \`),actions:[{label:"Cancel",variant:"btn-outline"},{label:"Generate",primary:!0,variant:"btn-primary",onClick:async e=>{await jt(e)}}]})}async function jt(e){const t=e.element,r=parseInt(t.querySelector("#gen-days").value,10),n=t.querySelector("#gen-goal").value,a=t.querySelector("#gen-custom").value.trim(),o=Array.from(t.querySelectorAll('input[name="equipment"]:checked')).map(c=>c.value),s=t.querySelector(".modal-body");s.innerHTML=String(i\`
  ...
  \`);const l=t.querySelector(".modal-footer");l&&(l.style.display="none");try{await p.post("/programs/generate",{days_per_week:r,goal:n,custom_instructions:a,available_equipment:o}),s.innerHTML=String(i\`
      <div class="generate-progress">
    ...
    \`),setTimeout(()=>{e.close(),F()},1500)}catch(c){s.innerHTML=String(i\`
      <div class="generate-progress">
    ...
    \`),l&&(l.style.display="")}}const d={name:"",days_per_week:3,goal:"hypertrophy",days:[],currentDayIndex:0,availableExercises:[],modalApi:null};async function Le(){d.days=[],d.currentDayIndex=0;try{const e=await p.get("/exercises");d.availableExercises=e.exercises||[]}catch{g.error("Error loading exercises");return}Zt()}function Zt(){d.modalApi&&d.modalApi.close(),d.modalApi=C({title:"Create Custom Program",size:"wide",content:String(i\`
      ...
      <div class="form-group">
        ...
        <input type="text" id="builder-name" class="input" placeholder="e.g., My Push/Pull/Legs" value="\${d.name}" />
      </div>
      ...
      <div class="form-group">
        ...
        <select id="builder-days" class="select">
          \${[3,4,5,6].map(e=>i\`
            <option value="\${e}" \${d.days_per_week===e?"selected":""}>\${e} days</option>
          \`)}
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <select id="builder-goal" class="select">
          <option value="hypertrophy" \${d.goal==="hypertrophy"?"selected":""}>Hypertrophy (Muscle Growth)</option>
          <option value="strength" \${d.goal==="strength"?"selected":""}>Strength</option>
          <option value="endurance" \${d.goal==="endurance"?"selected":""}>Endurance</option>
        </select>
      </div>
    \`),actions:[{label:"Cancel",variant:"btn-outline"},{label:"Next: Configure Days",primary:!0,onClick:e=>{const t=e.element.querySelector("#builder-name").value.trim(),r=parseInt(e.element.querySelector("#builder-days").value,10),n=e.element.querySelector("#builder-goal").value;if(!t){g.warning("Please enter a program name");return}d.name=t,d.days_per_week=r,d.goal=n,d.days=Array.from({length:r},(a,o)=>({day_number:o+1,name:\`Day \${o+1}\`,focus:"",exercises:[]})),d.currentDayIndex=0,Z()}}]})}function Z(){const e=d.days[d.currentDayIndex],t=Jt(d.availableExercises),r=d.currentDayIndex===d.days_per_week-1;d.modalApi&&d.modalApi.close(),d.modalApi=C({title:"Create Custom Program",size:"wide",content:String(i\`
      <h3 style="margin-bottom: var(--space-1);">Day \${e.day_number} of \${d.days_per_week}</h3>
    ...
    \`),actions:[...d.currentDayIndex>0?[{label:"← Previous Day",variant:"btn-outline",onClick:()=>{ae(),d.currentDayIndex--,Z()}}]:[],{label:"Cancel",variant:"btn-ghost"},{label:r?"Finish & Save":"Next Day →",primary:!0,onClick:async()=>{if(ae(),e.exercises.length===0){g.warning("Please select at least one exercise for this day");return}r?await tr():(d.currentDayIndex++,Z())}}],onOpen:({element:n})=>Yt(n)})}function ae(){var a;const e=(a=d.modalApi)==null?void 0:a.element;if(!e)return;const t=d.days[d.currentDayIndex],r=e.querySelector("#builder-day-name"),n=e.querySelector("#builder-day-focus");r&&(t.name=r.value.trim()||\`Day \${t.day_number}\`),n&&(t.focus=n.value.trim())}function Jt(e){const t={};for(const r of e){const n=r.muscle_group||"Other";t[n]||(t[n]=[]),t[n].push(r)}return t}function Qt(e,t){const r=t.exercises.find(n=>n.exercise_id===e.id);return i\`
  ...
  \`}function Yt(e){e.addEventListener("change",t=>{const r=t.target.closest("[data-builder-toggle]");if(r){const a=parseInt(r.getAttribute("data-builder-toggle"),10);Xt(a);return}const n=t.target.closest("[data-builder-config]");if(n){const[a,o]=n.getAttribute("data-builder-config").split(":");er(parseInt(a,10),o,t.target.value)}})}function Xt(e){ae();const t=d.days[d.currentDayIndex],r=t.exercises.findIndex(n=>n.exercise_id===e);if(r>=0)t.exercises.splice(r,1);else{const n=d.availableExercises.find(a=>a.id===e);t.exercises.push({exercise_id:e,name:(n==null?void 0:n.name)||"",sets:3,reps:"8-12",rest_seconds:90})}Z()}function er(e,t,r){const a=d.days[d.currentDayIndex].exercises.find(o=>o.exercise_id===e);a&&(t==="reps"?a.reps=r:a[t]=parseInt(r,10)||0)}async function tr(){var e;try{await p.post("/programs/manual",{name:d.name,days_per_week:d.days_per_week,goal:d.goal,days:d.days}),g.success("Custom program created!"),(e=d.modalApi)==null||e.close(),d.modalApi=null,F()}catch(t){g.error(\`Error creating program: \${t.message}\`)}}async function rr(e){const t=window.__legacyStartWorkoutFromProgram;if(typeof t=="function")return t(e);typeof window.switchTab=="function"&&window.switchTab("workout")}function te(e){if(e===0||e==null)return i\`<span class="text-muted">—</span>\`;const r=e>0?"text-success":"text-danger";return i\`<span class="\${r}" style="font-weight: var(--font-semibold);">\${pt(e)}</span>\`}function me(e,t){const r=n=>z(n,{month:"short",day:"numeric"});return\`\${r(e)} – \${r(t)}\`}function ye(e,t,r){if(!r)return"";const{current:n,previous:a,changes:o}=r;return i\`
    <div style="margin-bottom: var(--space-5);">
      ...
      <div class="comparison-date-row">
        <span class="text-primary" style="font-weight: var(--font-medium);">
          \${me(n.start_date,n.end_date)}
        </span>
        <span class="text-muted">vs</span>
        <span>\${me(a.start_date,a.end_date)}</span>
      </div>
      <div class="progress-comparison-grid">
        <div class="progress-comparison-card">
          ...
          <div class="progress-comparison-change">\${te(o.total_volume)}</div>
        </div>
        <div class="progress-comparison-card">
          <div class="progress-comparison-label">Workout Time</div>
          <div class="progress-comparison-values">
            <span class="progress-comparison-current">\${R(n.total_time)}</span>
            <span class="progress-comparison-previous">vs \${R(a.total_time)}</span>
          </div>
          <div class="progress-comparison-change">\${te(o.total_time)}</div>
        </div>
        <div class="progress-comparison-card">
          ...
          <div class="progress-comparison-change">\${te(o.workout_count)}</div>
        </div>
      </div>
    </div>
  \`}function nr(e){return i\`
    <div class="grid grid-cols-auto">
      <div class="stat-card">
        <div class="stat-label">Total Workouts</div>
        <div class="stat-value">\${e.total_workouts||0}</div>
  ...
  \`}function ar(e){return!e||e.length===0?i\`
      <div class="empty-state">
        <div class="empty-state-icon">📊</div>
  ...
  \`}function ir(e){return!e||e.length===0?i\`
      <div class="empty-state">
  ...
  \`}function or(e){return!e||e.length===0?i\`
      <div class="empty-state">
  ...
  \`}function sr(e){return!e||e.length===0?i\`
      <div class="empty-state">
          ...
          <div class="pr-row">
            <div>
              ...
              <span class="text-muted" style="font-size: var(--text-xs);">
                \${String(t.record_type||"").toUpperCase()} · \${z(t.achieved_at)}
              </span>
            </div>
            <div style="text-align: right;">
  ...
  \`}async function cr(){if(await Y("This will recalculate all your Personal Records from your workout history. Continue?",{title:"Recalculate PRs?",confirmLabel:"Recalculate",confirmVariant:"btn-primary"}))try{const t=await p.post("/achievements/prs/recalculate");g.success(t.message||"PRs recalculated"),qe()}catch(t){g.error(\`Failed to recalculate PRs: \${t.message}\`)}}function pr(e){e.addEventListener("click",t=>{if(t.target.closest('[data-action="recalc-prs"]')){cr();return}t.target.closest('[data-action="open-exporter"]')&&(typeof window.openUnifiedExporter=="function"?window.openUnifiedExporter():g.info("Export center is not yet available"))})}async function qe(){const e=document.getElementById("analytics");if(e){e.innerHTML=String(lr());try{const[t,r,n,a,o]=await Promise.all([p.get("/analytics/progress?days=90"),p.get("/analytics/volume?days=90&group_by=week"),p.get("/analytics/bodymap?days=7"),p.get("/achievements/prs?limit=10"),p.get("/analytics/progress-comparison")]),s=a.prs||[],l=t.overview||{},c=r.volume_by_muscle||[],u=n.body_map||[],m=t.top_exercises||[];e.innerHTML=String(i\`
      <div class="stack stack-lg">
        <div class="card">
          <div class="card-header">
            <h2 class="card-title"><i class="fas fa-chart-line"></i> Progress Comparison</h2>
          </div>
          \${ye("This Week vs Last Week","fa-calendar-week",o.weekly)}
          \${ye("This Month vs Last Month","fa-calendar-alt",o.monthly)}
        </div>
    ...
    \`),pr(e);const y=window.loadWorkoutHistory;if(typeof y=="function")try{await y(e)}catch(S){console.warn("Legacy loadWorkoutHistory failed:",S)}}catch(t){console.error("Error loading analytics:",t),e.innerHTML=String(dr(t.message))}}}function gr(e){return i\`
    <div class="macro-rings">
      <div class="macro-ring-wrapper">
        \${A(U({value:Math.round(e.protein_grams||0),max:Math.round(e.protein_goal||150),size:120,unit:"g",label:"Protein",color:"var(--color-secondary)"}))}
      ...
      <div class="macro-ring-wrapper">
        \${A(U({value:Math.round(e.water_ml||0),max:Math.round(e.water_goal||2500),size:120,unit:"ml",label:"Water",color:"var(--color-primary)"}))}
        <div class="macro-ring-meta">
      ...
      <div class="macro-ring-wrapper">
        \${A(U({value:Math.round((e.creatine_grams||0)*10)/10,max:e.creatine_goal||5,size:120,unit:"g",label:"Creatine",color:"#8b5cf6"}))}
        <div class="macro-ring-meta">
  ...
  \`:""}function vr(e,t){if(!e||e.length===0)return i\`
      <div class="empty-state">
    ...
    <div class="nutrition-bar-chart">
      \${r.map(n=>{const a=Math.min(n.protein/n.protein_goal*100,100),o=Math.min(n.water/n.water_goal*100,100),s=n.date===new Date().toISOString().split("T")[0];return i\`
          <div class="nutrition-bar-day \${s?"is-today":""}">
            <div class="nutrition-bar-label">\${z(n.date,{month:"short",day:"numeric"})}</div>
            <div class="nutrition-bars">
              ...
              <div class="nutrition-bar-track" title="Water">
                <div class="nutrition-bar-fill \${n.hit_water?"hit":""}" style="height: \${o}%; background: var(--color-primary);"></div>
              </div>
            </div>
  ...
  \`}function D(e,...t){const r=window[e];if(typeof r=="function")try{return r(...t)}catch(n){g.error(\`Error: \${n.message}\`)}else g.warning("This feature is not yet available.")}async function br(){const e=parseFloat(document.getElementById("nq-protein").value)||0,t=parseFloat(document.getElementById("nq-water").value)||0,r=parseFloat(document.getElementById("nq-creatine").value)||0;if(!e&&!t&&!r){g.warning("Enter an amount in at least one field.");return}try{const n=[];e>0&&n.push(p.post("/nutrition/entries",{entry_type:"protein",amount:e,unit:"g"})),t>0&&n.push(p.post("/nutrition/entries",{entry_type:"water",amount:t,unit:"ml"})),r>0&&n.push(p.post("/nutrition/entries",{entry_type:"creatine",amount:r,unit:"g"})),await Promise.all(n),document.getElementById("nq-protein").value="",document.getElementById("nq-water").value="",document.getElementById("nq-creatine").value="",g.success("Nutrition logged!"),He()}catch(n){g.error(\`Error logging nutrition: \${n.message}\`)}}function fr(e){e.addEventListener("click",t=>{const r=t.target.closest("[data-action]");if(!r)return;switch(r.getAttribute("data-action")){case"log-meal":D("showLogMealModal",r.getAttribute("data-meal-type"));break;case"scan-barcode":D("showBarcodeScanner");break;case"quick-macros":D("showQuickMacroEntry");break;case"saved-meals":D("showSavedMeals");break;case"log-all-quick":br();break;case"view-entries":D("loadNutritionEntries");break;case"create-saved":D("showCreateSavedMeal");break}})}function xr(){return i\`
    <div class="stack stack-lg">
      <div class="card"><div class="skeleton skeleton-card"></div></div>
      <div class="card"><div class="skeleton skeleton-card"></div></div>
    </div>
  \`}function wr(e){return i\`
    <div class="card">
      <div class="empty-state">
        <div class="empty-state-icon">⚠️</div>
  ...
  \`}async function He(){const e=document.getElementById("nutrition");if(e){e.innerHTML=String(xr());try{const[t,r,n]=await Promise.all([p.get("/nutrition/daily"),p.get("/nutrition/analytics?days=30"),p.get("/nutrition/streaks")]),a=r.summary,o=r.daily_data||[];e.innerHTML=String(i\`
      <div class="stack stack-lg">
        <div class="card">
          <div class="card-header">
    ...
    \`),fr(e)}catch(t){console.error("Error loading nutrition:",t),e.innerHTML=String(wr(t.message))}}}const kr=["How should I progress my bench press?","Am I training with enough volume?","What muscle groups am I neglecting?","Should I take a deload week?","How do I break through a plateau?","Is my training frequency optimal?"];function Sr(e){const t=document.createElement("div");return t.textContent=e||"",t.innerHTML}function Be(e){return!e||e.length===0?i\`
    ...
    \${e.map(t=>i\`
      <div class="chat-message chat-message-\${t.role}">
        ...
        <div class="chat-message-body">\${A(\$r(t.content))}</div>
      </div>
    \`)}
  \`}function \$r(e){if(!e)return"";let t=Sr(e);return t=t.replace(/\\*\\*(.+?)\\*\\*/g,"<strong>\$1</strong>"),t=t.replace(/^\\s*-\\s+(.+)\$/gm,"<li>\$1</li>"),t=t.replace(/(<li>.*<\\/li>\\n?)+/g,r=>\`<ul>\${r}</ul>\`),t=t.replace(/\\n(?!<\\/?(ul|li)>)/g,"<br />"),t}async function he(e){var a,o;const t=w.get("aiCoach.conversationHistory")||[],r=[...t,{role:"user",content:e}];w.set("aiCoach.conversationHistory",r),ie();const n=document.getElementById("ai-chat-messages");n&&(n.insertAdjacentHTML("beforeend",String(i\`
    ...
    \`)),n.scrollTop=n.scrollHeight);try{const s=await p.post("/ai/coach/chat",{message:e,conversationHistory:t.slice(-6)});(a=document.getElementById("chat-typing"))==null||a.remove();const l={role:"assistant",content:s.response||"Sorry, I could not generate a response."};w.set("aiCoach.conversationHistory",[...r,l]),ie()}catch(s){(o=document.getElementById("chat-typing"))==null||o.remove(),g.error(\`AI Coach error: \${s.message}\`)}}function ie(){const e=document.getElementById("ai-chat-messages");if(!e)return;const t=w.get("aiCoach.conversationHistory")||[];e.innerHTML=String(Be(t)),e.scrollTop=e.scrollHeight}function Tr(e){const t=e.querySelector("#ai-chat-form");t&&t.addEventListener("submit",r=>{r.preventDefault();const n=t.querySelector("#ai-chat-input"),a=n.value.trim();a&&(n.value="",he(a))}),e.addEventListener("click",r=>{const n=r.target.closest("[data-action]");if(!n)return;switch(n.getAttribute("data-action")){case"quick-question":he(n.getAttribute("data-question"));break;case"clear-chat":w.set("aiCoach.conversationHistory",[]),ie();break;case"open-analysis":typeof window.generateAICoaching=="function"?window.generateAICoaching():g.info("Full analysis is not yet available.");break;case"open-advanced":typeof window.loadAdvancedAnalytics=="function"?window.loadAdvancedAnalytics():g.info("Advanced analytics is not yet available.");break}})}function _r(){return i\`
  ...
  \`}async function Cr(){var t,r;const e=document.getElementById("insights");if(e){e.innerHTML=String(_r());try{const a=(await p.get("/ai/coach/analysis?days=90")).data;if(!a||(((t=a.summary)==null?void 0:t.total_workouts)||0)<1){e.innerHTML=String(Mr()),(r=e.querySelector('[data-action="go-workout"]'))==null||r.addEventListener("click",()=>{typeof window.switchTab=="function"&&window.switchTab("workout")});return}w.set("aiCoach.lastInsight",a);const o=w.get("aiCoach.conversationHistory")||[];e.innerHTML=String(i\`
      <div class="stack stack-lg">
        <div class="card ai-coach-hero">
          <div class="ai-coach-hero-content">
            <h2><i class="fas fa-robot"></i> AI Fitness Coach</h2>
        ...
        <div class="card">
          <div class="card-header">
            ...
            \${o.length>0?i\`<button class="btn btn-ghost btn-sm" data-action="clear-chat"><i class="fas fa-trash"></i> Clear</button>\`:""}
          </div>

          <div class="chat-messages" id="ai-chat-messages">\${Be(o)}</div>
        ...
        <div class="card">
          ...
          <div class="quick-questions-grid">
            \${kr.map(s=>i\`
              <button class="quick-question-btn" data-action="quick-question" data-question="\${s}">
                \${s}
              </button>
            \`)}
          </div>
    ...
    \`),Tr(e)}catch(n){console.error("Error loading AI coach:",n),e.innerHTML=String(zr(n.message))}}}const ve={primed:{label:"Primed to Go",emoji:"🔥",color:"var(--color-secondary)"},ready:{label:"Ready",emoji:"✅",color:"var(--color-primary)"},fatigued:{label:"Fatigued",emoji:"😮‍💨",color:"var(--color-warning)"},needs_rest:{label:"Needs Rest",emoji:"😴",color:"var(--color-danger)"}};function Er(e){const t=ve[e.status]||ve.ready;return i\`
    <div class="preview-readiness">
      <div class="preview-readiness-ring">
        \${A(U({value:e.score,max:100,size:140,unit:"",label:"Readiness",color:t.color}))}
      </div>
  ...
  \`}function Rr(e){const t=Ee(),r=e.suggested_weight_kg!=null?\`\${Math.round(Re(e.suggested_weight_kg)*10)/10} \${t}\`:"No data yet",n=e.confidence==="high"?"confidence-high":e.confidence==="medium"?"confidence-medium":"confidence-low";return i\`
  ...
  \`}function Pr(e){return!e||e.length===0?i\`
      <div class="empty-state">
  ...
  \`}async function Ir(e,t={}){g.info("Preparing your AI briefing…",{duration:2e3});try{const n=(await p.get(\`/ai/realtime/preview/\${e}\`)).data;return await new Promise(a=>{C({title:"Today's Workout Preview",size:"wide",content:String(i\`
          <div class="stack stack-lg">
            ...
            \${n.target_muscles&&n.target_muscles.length>0?i\`
                  <div>
                    ...
                    <div class="cluster">
                      \${n.target_muscles.map(o=>i\`<span class="badge badge-primary">\${o}</span>\`)}
                    </div>
                  </div>
                \`:""}
        ...
        \`),actions:[{label:"Skip Briefing",variant:"btn-outline",onClick:o=>{o.close("cancel"),a("cancel")}},{label:"Start Workout",primary:!0,variant:"btn-primary",onClick:o=>{var s;o.close("start"),(s=t.onStart)==null||s.call(t),a("start")}}]})})}catch(r){return console.error("Workout preview error:",r),g.error(\`Couldn't load briefing: \${r.message}\`),"cancel"}}class Ar{constructor(){this.listeners=new Map}on(t,r){return this.listeners.has(t)||this.listeners.set(t,new Set),this.listeners.get(t).add(r),()=>this.off(t,r)}once(t,r){const n=a=>{this.off(t,n),r(a)};return this.on(t,n)}off(t,r){var n;if(!r){this.listeners.delete(t);return}(n=this.listeners.get(t))==null||n.delete(r)}emit(t,r){const n=this.listeners.get(t);if(n)for(const a of n)try{a(r)}catch(o){console.error(\`Event handler error for "\${t}":\`,o)}}}const Lr=new Ar;let x=null,k=!1,v=null,J=null;const be={progression:{icon:"fa-arrow-up",color:"var(--color-secondary)",title:"Great progress!"},on_track:{icon:"fa-check-circle",color:"var(--color-primary)",title:"On target"},complete:{icon:"fa-trophy",color:"var(--color-gold)",title:"Crushed it!"},form_warning:{icon:"fa-exclamation-triangle",color:"var(--color-warning)",title:"Form watch"},load_warning:{icon:"fa-weight-hanging",color:"var(--color-warning)",title:"Weight watch"}};function De(){if(!v)return i\`
      <div class="live-coach-overlay \${k?"is-collapsed":""}" role="status">
        <button class="live-coach-toggle" data-action="toggle" aria-label="\${k?"Expand":"Collapse"} AI coach">
          ...
          \${k?"":i\`
              ...
              \`}
          <i class="fas \${k?"fa-chevron-up":"fa-chevron-down"} live-coach-chevron"></i>
        </button>
      </div>
    \`;const e=be[v.type]||be.on_track;return i\`
    <div class="live-coach-overlay has-tip \${k?"is-collapsed":""}" role="status">
      <button class="live-coach-toggle" data-action="toggle">
        ...
        \${k?i\`<div class="live-coach-subtitle">\${e.title}</div>\`:i\`
              <div class="live-coach-body">
                <div class="live-coach-title">\${e.title}</div>
                <div class="live-coach-subtitle">\${v.message}</div>
              </div>
            \`}
        <i class="fas \${k?"fa-chevron-up":"fa-chevron-down"} live-coach-chevron"></i>
      </button>
  ...
  \`}function V(){if(!x)return;const e=Q(De());x.replaceWith(e),x=e}function Fe(e){const t=e.target.closest("[data-action]");if(!t)return;switch(t.getAttribute("data-action")){case"toggle":k=!k,V();break;case"apply-tip":J&&(v!=null&&v.action)&&J(v.action),v=null,V();break;case"dismiss-tip":v=null,V();break}}function qr(e={}){x&&document.body.contains(x)||(J=e.onAction,x=Q(De()),x.addEventListener("click",Fe),document.body.appendChild(x))}function Hr(){x&&(x.removeEventListener("click",Fe),x.remove(),x=null),v=null,k=!1,J=null}async function Br(e){if(x)try{const t=await p.post("/ai/realtime/analyze",{current_sets:e.currentSets,target_reps:e.targetReps,target_sets:e.targetSets});t.insight&&(v=t.insight,k=!1,V(),!t.insight.action&&t.insight.type!=="form_warning"&&t.insight.type!=="load_warning"&&setTimeout(()=>{v===t.insight&&(v=null,V())},8e3),Lr.emit("aiCoach:tip",t.insight))}catch(t){console.warn("AI coach analysis failed:",t)}}const re={init:qr,destroy:Hr,analyzeSet:Br};window.__fitnessApp={store:w,api:p,toast:g,openModal:C,confirmDialog:Y};window.aiCoach={showPreview:Ir,initLive:re.init,destroyLive:re.destroy,analyzeSet:re.analyzeSet};f("learn","loadLearn",at);f("achievements","loadAchievements",\$t);f("dashboard","loadDashboard",Pt);f("profile","showProfile",Dt);f("program","loadPrograms",Pe);f("programs:view","viewProgram",oe);f("programs:activate","activateProgram",se);f("programs:delete","deleteProgram",Kt);f("programs:generate","showGenerateProgram",Ae);f("programs:manual","showCreateManualProgram",Le);f("programs:rename","showRenameProgramModal",Ie);f("analytics","loadAnalytics",qe);f("nutrition","loadNutrition",He);f("insights","loadInsights",Cr);et();
//# sourceMappingURL=index-DAXguq2N.js.map
