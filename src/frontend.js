...
(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function o(s){if(s.ep)return;s.ep=!0;const r=n(s);fetch(s.href,r)}})();const N=Symbol("raw"),z=Symbol("trusted");class k{constructor(e){this[z]=!0,this.value=String(e)}toString(){return this.value}}function D(t){return t==null||t===!1?new k(""):new k(t)}function Y(t){return String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Q(t){return String(t).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;")}function R(t){return t==null||t===!1?"":t===!0?"true":t instanceof k?t.value:t&&typeof t=="object"&&N in t?t[N]:Array.isArray(t)?t.map(R).join(""):Y(t)}function T(t,...e){let n=t[0];for(let o=0;o<e.length;o++)n+=R(e[o])+t[o+1];return new k(n)}function K(t){const e=t instanceof k?t.value:String(t).trim(),n=document.createElement("template");return n.innerHTML=e,n.content.firstElementChild}const U="fitness-coach:state",J={user:null,currentWorkout:null,currentProgram:null,currentTab:"dashboard",ui:{theme:"system",activeModal:null,lastVisitedTab:null},workout:{activeExerciseIndex:0,restTimerEndTime:null,workoutStartTime:null,exerciseHistory:{},modifications:{added:[],deleted:[]},pendingNotes:""},mealTracking:{searchResults:[],selectedFoods:[],currentMealType:null,scannerActive:!1},pastWorkout:{date:null,programId:null,programDayId:null,exercises:[]},analytics:{lastFetch:null,data:null},aiCoach:{conversationHistory:[],lastInsight:null}},W=["ui"];function M(t,e){return e?e.split(".").reduce((n,o)=>n==null?n:n[o],t):t}function G(t,e,n){if(!e)return n;const o=e.split("."),s=o.pop(),r=o.reduce((i,a)=>((i[a]==null||typeof i[a]!="object")&&(i[a]={}),i[a]),t);return r[s]=n,t}function q(t){return t==null||typeof t!="object"?t:JSON.parse(JSON.stringify(t))}function X(){try{const t=localStorage.getItem(U);return t?JSON.parse(t):{}}catch{return{}}}function Z(t){try{const e={};for(const n of W){const o=M(t,n);o!==void 0&&G(e,n,o)}localStorage.setItem(U,JSON.stringify(e))}catch{}}class tt{constructor(e){this.data=q(e),this.subscribers=new Map;const n=X();for(const[o,s]of Object.entries(n))this.data[o]={...this.data[o],...s}}get(e){return M(this.data,e)}set(e,n){const o=this.get(e);if(o===n)return;G(this.data,e,n),W.some(r=>e===r||e.startsWith(\`\${r}.\`))&&Z(this.data),this.notify(e,n,o);const s=e.split(".");for(let r=s.length-1;r>0;r--){const i=s.slice(0,r).join(".");this.notify(i,this.get(i),null)}}merge(e,n){const o=this.get(e)||{};this.set(e,{...o,...n})}reset(e){const n=M(J,e);this.set(e,q(n))}subscribe(e,n){this.subscribers.has(e)||this.subscribers.set(e,new Set);const o=this.subscribers.get(e);return o.add(n),()=>o.delete(n)}notify(e,n,o){const s=this.subscribers.get(e);if(s)for(const r of s)try{r(n,o)}catch(i){console.error(\`Subscriber error for "\${e}":\`,i)}}}const j=new tt(J);typeof window<"u"&&(window.__store=j);const V="/api",et=3e4,nt=6e4,I=new Map;class P extends Error{constructor(e,{status:n=0,code:o=null,details:s=null}={}){super(e),this.name="ApiError",this.status=n,this.code=o,this.details=s}}async function C(t,e={}){var L;const{method:n="GET",body:o,headers:s={},cache:r=!1,timeout:i=et,signal:a}=e,b=\`\${V}\${t}\`,p=\`\${n} \${b}\`;if(n==="GET"&&r){const m=I.get(p);if(m&&Date.now()-m.at<nt)return m.data}const y=new AbortController,l=setTimeout(()=>y.abort(),i);a&&a.addEventListener("abort",()=>y.abort(),{once:!0});const h={method:n,headers:{"Content-Type":"application/json",...s},signal:y.signal};o!=null&&(o instanceof FormData?(delete h.headers["Content-Type"],h.body=o):typeof o=="string"?h.body=o:h.body=JSON.stringify(o));let f;try{f=await fetch(b,h)}catch(m){throw clearTimeout(l),m.name==="AbortError"?new P("Request timed out",{code:"TIMEOUT"}):new P("Network error",{code:"NETWORK",details:m.message})}clearTimeout(l);const g=(L=f.headers.get("content-type"))==null?void 0:L.includes("application/json"),d=g?await f.json().catch(()=>({})):await f.text();if(!f.ok){const m=g&&(d==null?void 0:d.error)||\`Request failed (\${f.status})\`;throw new P(m,{status:f.status,code:d==null?void 0:d.code,details:d==null?void 0:d.details})}return n==="GET"&&r&&I.set(p,{data:d,at:Date.now()}),d}function ot(t=""){const e=\`GET \${V}\${t}\`;for(const n of I.keys())n.startsWith(e)&&I.delete(n)}const rt={get:(t,e)=>C(t,{...e,method:"GET"}),post:(t,e,n)=>C(t,{...n,method:"POST",body:e}),put:(t,e,n)=>C(t,{...n,method:"PUT",body:e}),patch:(t,e,n)=>C(t,{...n,method:"PATCH",body:e}),delete:(t,e)=>C(t,{...e,method:"DELETE"}),request:C,invalidateCache:ot},H={success:"fa-check-circle",error:"fa-exclamation-circle",warning:"fa-exclamation-triangle",info:"fa-info-circle"},st=3e3;let u=null;function it(){return u&&document.body.contains(u)||(u=document.getElementById("toast-container"),u||(u=document.createElement("div"),u.id="toast-container",u.setAttribute("aria-live","polite"),u.setAttribute("aria-atomic","true"),document.body.appendChild(u))),u}function \$(t,e={}){var y;const{type:n="info",duration:o=st,icon:s,action:r}=e,i=s?\`fa-\${s}\`:H[n]||H.info,a=K(T\`
    <div class="toast toast-\${Q(n)}" role="alert">
      <i class="fas \${i} toast-icon" aria-hidden="true"></i>
      <div class="toast-message">\${t}</div>
      \${r?T\`<button class="btn btn-sm btn-ghost toast-action">\${r.label}</button>\`:""}
    </div>
  \`);it().appendChild(a),r!=null&&r.onClick&&((y=a.querySelector(".toast-action"))==null||y.addEventListener("click",()=>{r.onClick(),p()}));let b=!1;const p=()=>{b||(b=!0,a.style.animation="slide-out-right var(--transition-base) forwards",a.addEventListener("animationend",()=>a.remove(),{once:!0}))};return o>0&&setTimeout(p,o),p}const at={show:(t,e)=>\$(t,e),success:(t,e)=>\$(t,{...e,type:"success"}),error:(t,e)=>\$(t,{...e,type:"error",duration:(e==null?void 0:e.duration)??5e3}),warning:(t,e)=>\$(t,{...e,type:"warning"}),info:(t,e)=>\$(t,{...e,type:"info"})},A=[];function ct(){let t=document.getElementById("modal-root");return t||(t=document.createElement("div"),t.id="modal-root",document.body.appendChild(t)),t}function lt(t){const e=t.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])');if(e.length===0)return null;const n=e[0],o=e[e.length-1],s=r=>{r.key==="Tab"&&(r.shiftKey&&document.activeElement===n?(r.preventDefault(),o.focus()):!r.shiftKey&&document.activeElement===o&&(r.preventDefault(),n.focus()))};return t.addEventListener("keydown",s),n.focus(),()=>t.removeEventListener("keydown",s)}function B(t){var x;const{title:e,content:n,actions:o=[],size:s="default",dismissable:r=!0,onOpen:i,onClose:a}=t,b=s==="wide"?"modal-wide":s==="fullscreen"?"modal-fullscreen":"",p=o.length?T\`
        <div class="modal-footer">
          \${o.map((c,w)=>T\`
            <button
              class="btn \${c.variant||(c.primary?"btn-primary":"btn-outline")}"
              data-modal-action="\${w}"
            >\${c.label}</button>
          \`)}
        </div>
    ...
    <div class="modal-backdrop" role="dialog" aria-modal="true">
      <div class="modal \${b}" role="document">
        \${D(y)}
        <div class="modal-body"></div>
        \${D(p)}
      </div>
    </div>
  \`),h=l.querySelector(".modal"),f=l.querySelector(".modal-body");n instanceof Node?f.appendChild(n):n!=null&&(f.innerHTML=String(n));const g=document.activeElement;let d=!1,L;const m=new Promise(c=>L=c),v=c=>{if(d)return;d=!0;const w=A.indexOf(E);w>=0&&A.splice(w,1),O==null||O(),l.style.animation="fade-out var(--transition-fast) forwards",h.style.animation="slide-up var(--transition-fast) reverse forwards",l.addEventListener("animationend",()=>{var S;l.remove(),(S=g==null?void 0:g.focus)==null||S.call(g)},{once:!0}),A.length===0&&(document.body.style.overflow=""),a==null||a(c),L(c)},E={element:h,backdrop:l,close:v,promise:m};l.querySelectorAll("[data-modal-action]").forEach(c=>{const w=parseInt(c.getAttribute("data-modal-action"),10),S=o[w];c.addEventListener("click",async()=>{S.onClick?await S.onClick(E):v(S.value??w)})}),(x=l.querySelector("[data-modal-close]"))==null||x.addEventListener("click",()=>v()),r&&l.addEventListener("click",c=>{c.target===l&&v()});const _=c=>{c.key==="Escape"&&r&&A[A.length-1]===E&&v()};document.addEventListener("keydown",_);const F=v;E.close=c=>{document.removeEventListener("keydown",_),F(c)},ct().appendChild(l),document.body.style.overflow="hidden",A.push(E);const O=lt(h);return i==null||i(E),E}function dt(t,e={}){const{title:n="Are you sure?",confirmLabel:o="Confirm",cancelLabel:s="Cancel",confirmVariant:r="btn-primary"}=e;return new Promise(i=>{B({title:n,content:T\`<p>\${t}</p>\`,actions:[{label:s,variant:"btn-outline",onClick:a=>a.close(!1)},{label:o,variant:r,primary:!0,onClick:a=>a.close(!0)}],onClose:a=>i(a===!0)})})}window.__fitnessApp={store:j,api:rt,toast:at,openModal:B,confirmDialog:dt};
//# sourceMappingURL=index-B5e2yWc6.js.map
