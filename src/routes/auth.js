...
auth.put('/user', async (c) => {
  ...
  const user = await db.prepare(
    `UPDATE users 
     SET age = ?, height_cm = ?, weight_kg = ?, name = ?, gender = ?, measurement_system = ?, updated_at = CURRENT_TIMESTAMP 
     WHERE email = ? 
     RETURNING *`
  ).bind(age, height_cm, weight_kg, name, gender || 'not_specified', measurement_system || 'metric', payload.email).first();
