...
// Note: the old backend `analyzePostSet()` and its LLM-powered orchestrator
// `analyzeSetWithContext()` were removed when the in-workout coaching surface
// moved to a frontend rule analyzer. See
// `frontend/src/features/active-workout/set-analyzer.js`. The history-fetch
// helpers that fed those functions also moved — the data is now served by
// `GET /api/workouts/exercises/:id/context` (see src/routes/workouts.js).
